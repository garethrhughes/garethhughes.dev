...
### Timeline design language rolled across the remaining pages
- Decision: Extracted `TagPill`, `PageHeader`/`SectionLabel` and `Rail`/`RailRow` as shared primitives, refitted the home timeline onto them, and applied them to the post detail page, `/about/`, `/projects/`, `/archive/` and the footer. Cards are kept only where `docs/STYLE_GUIDE.md` sanctions them (project tiles, the earlier-roles table) and flattened from `shadow-sm` -> `shadow-md` to `border` + `hover:border-squirrel-300`. Added older/newer navigation and a `max-w-[72ch]` measure cap to posts, and the missing Open Graph / Twitter metadata to `/about/` and `/projects/`.
- Why: ADR 0016 gave `/` a distinct vocabulary and left the other five pages speaking the old one. Worst case: the post detail page's related-posts aside was still rendering the `PostCard` that ADR 0016 deleted — shadow lift and image zoom included — on the page every timeline click lands on. Four passive pill styles and three page-header treatments existed because there was never one definition to import.
- Scope: `components/TagPill.tsx`, `components/PageHeader.tsx`, `components/Rail.tsx` (all new), `app/posts/[slug]/page.tsx`, `app/about/page.tsx`, `app/projects/page.tsx`, `app/archive/page.tsx`, `components/ArchiveList.tsx`, `components/Timeline.tsx`, `components/TimelineItem.tsx`, `components/Footer.tsx`, `lib/posts.ts`.
- Notes: ADR `docs/decisions/0017-design-language-rollout.md`, proposal `docs/proposals/0004-design-language-rollout.md`. Worth remembering: (1) `Rail` is geometry only — gutter, border, dot, the `display: contents` row wrapper — and everything else is children; that is what lets one component serve the post timeline, a CV and a related-posts list without a mode flag per caller. (2) About's Skills section deliberately does *not* use `Rail`: category names like "Cloud & Infrastructure" do not fit the 96px gutter, so it uses a `md:w-52` label column instead. (3) The loud filled pill is now reserved for the *active* state of an interactive chip; `TagPill` is the passive treatment everywhere. (4) The archive row synopsis uses `lead[0]`, not `excerpt` — `excerpt` retains a post's H1, so rows read "OpenCode Skills OpenCode Skills Getting hands-on with...". Same reason the timeline uses it. (5) The post page container is `max-w-[1140px]` rather than `max-w-7xl` so the grid column *is* the 72ch measure; capping inside a wider column left ~140px of dead space that read as an enormous gutter beside the aside. (6) Follow-ups left open: share project identity between `/about/` and `lib/projects.ts`, retune `.note-preview` for long-form reading, and decide whether "Blog" in the nav should read "Writing".
## 2026-08-20
